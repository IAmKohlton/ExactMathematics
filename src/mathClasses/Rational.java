...
public class Rational implements Comparable<Rational> {
    ...
    /**
     * raise a rational number to the kth power
     * @param k integer we raise to the power of
     * @return powered rational number
     */
    public Rational power(int k){
        if(this.equals(new Rational(0)) && k == 0){
            throw new ArithmeticException("Cannot raise 0 to 0th power");
        }
        if(this.isInfinity() && k == 0){
            throw new ArithmeticException("Cannot raise infinity to 0th power");
        }
        if(this.isInfinity() && k < 0){
            return new Rational(0);
        }else if(this.isInfinity() && k > 0){
            if(!this.getSign()){
                return makePositiveInfinity();
            }else{
                if(k % 2 == 0){
    ...
    private long pow(long rat, int k){
        long ratio = (long) rat;
        for (int i = 0; i < k; i++) {
            ratio = ratio * rat;
        }
