...
public class RationalPolynomial {
    ...
    public Rational solve(Rational k){
        Rational runningTotal = new Rational(0);
        Rational coeff;
        Rational power;
        this.poly.goFirst();
        for (int i = 0; i < this.poly.getSize(); i++) {
            coeff = this.currentRational();
            power = k.power(i);
            runningTotal.increment(coeff.multiply(power));
    ...
    public static void main(String[] args){
        // TODO do a real regression test on methods in toTest
